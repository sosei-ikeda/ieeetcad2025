...
// mode => 0:gradient descent, 1:ridge regression, 2:prediction
    ...
    if(mode==0){
        dfr.tikhonov_init();
        Loop1:for(int i=0; i<Ns; i++) {
        	Loop11:for (int j=0; j<Ny; j++) {
                dfr.D[j] = 0;
            }
            dfr.D[D[i]] = 1;
            Loop12:for(int j=0; j<Nt; j++){
#pragma HLS PIPELINE off
    	...
    	Loop2:for(int i=0; i<Ns; i++){
    		Loop21:for(int j=0; j<Nt; j++){
#pragma HLS PIPELINE
                dfr.update(&U[i*Nt*(Nu+1)+j*(Nu+1)]);
                if(U[i*Nt*(Nu+1)+j*(Nu+1)+Nu]!=0 or j==Nt-1){
                    Y_buff[i] = dfr.output();
        ...
        dfr.grad_init();
        Loop3:for(int epoch=0; epoch<mode; epoch++){
        	Loop31:for(int i=0; i<Ns; i++) {
        		Loop311:for (int j=0; j<Ny; j++) {
                    dfr.D[j] = 0;
                }
                dfr.D[D[IDX[epoch*Ns+i]]] = 1;
                Loop312:for(int j=0; j<Nt; j++){
#pragma HLS PIPELINE off
